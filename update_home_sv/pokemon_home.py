...
class pokemon_home:
    ...
    def __parse_pokemon_detail(self, detail_json: dict):
        ...
        return:
            ...
            output_terastype:テラスタイプに関するデータ詳細
        """
        output_move = []
        output_ability = []
        output_nature = []
        ...
        for pokemon_id, value in tqdm(detail_json.items()):
            ...
            for form_id, value_2 in value.items():
                ...
                output_nature.extend(
                    self.__output_detail(pokemon_info["seikaku"], pokemon_name, pokemon_id, form_id, self.NATURE)
                )
                output_item.extend(
                    self.__output_detail(pokemon_info["motimono"], pokemon_name, pokemon_id, form_id, self.ITEM)
                )
                output_pokemon.extend(
                    self.__output_detail_pokemon(pokemon_info["pokemon"], pokemon_name, pokemon_id, form_id)
                )
                output_terastype.extend(
                    self.__output_detail(pokemon_info["terastal"], pokemon_name, pokemon_id, form_id, self.TYPE)
                )
                
        return output_move, output_ability,output_nature,output_item,output_pokemon,output_terastype
    ...
    def __output_detail_pokemon(self, info_json: list, pokemon_name: str, pokemon_id: str, form_id: str):
        """ポケモンデータ詳細の入ったリストからデータを取得し、メタデータを付与して返す
        ...
        for i, info in enumerate(info_json):
            ...
            id=info["id"]+mkid(info["form"]+1)
            output.append([pokemon_name, pokemon_id, form_id, i + 1, id])
        return output
    ...
    def output_pokemon_detail(self):
        ...
        return:
            ...
            output_terastype:全ポケモンのテラスタイプに関するデータ詳細
        """
        output_move = []
        output_ability = []
        output_nature = []
        ...
        for i in range(1, 7):
            ...
            move, ability, nature, item, pokemon ,terastype = self.__parse_pokemon_detail(pokemon_all_detail)
            output_move.extend(move)
            output_ability.extend(ability)
            output_nature.extend(nature)
            output_item.extend(item)
            output_pokemon.extend(pokemon)
            output_terastype.extend(terastype)
        return output_move, output_ability,output_nature,output_item,output_pokemon,output_terastype
...
if __name__ == "__main__":
    ...
    move, ability,nature,item,pokemon,terastype = pokemon.output_pokemon_detail()
    ...
    pd.DataFrame(terastype, columns=["pokemon", "id", "form", "rank", "terastype", "raito"]).to_csv(
